...
def qq_download_by_vid(vid, title, output_dir='.', merge=True, info_only=False):
    ...
    best_quality = sorted(parts_formats, key=lambda part_format: part_format['fs'])[-1]['name']  # sort by fs--filesize,标清到高清，取末尾的高清
...
def qq_download(url, output_dir='.', merge=True, info_only=False, **kwargs):
    ...
    # do redirect
        ...
        title = match1(content, r'<a.*?id\s*=\s*"%s".*?title\s*=\s*"(.+?)".*?>' % vid) or match1(content, r'title">([^"]+)</p>') or match1(content, r'"title":"([^"]+)"') or vid
