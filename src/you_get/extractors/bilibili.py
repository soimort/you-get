...
class Bilibili(VideoExtractor):
    ...
    @staticmethod
    def bilibili_seasons_archives_api(mid, sid, pn=1, ps=100):
        return 'https://api.bilibili.com/x/polymer/space/seasons_archives_list?mid=%s&season_id=%s&page_num=%s&page_size=%s&sort_reverse=false' % (mid, sid, pn, ps)
    ...
    def download_playlist_by_url(self, url, **kwargs):
        ...
        if sort == 'video':
            ...
            mid, sid = m.group(1), m.group(2)
            api_url = self.bilibili_seasons_archives_api(mid, sid)
            api_content = get_content(api_url, headers=self.bilibili_headers(referer=self.url))
            archives_info = json.loads(api_content)
            # TBD: channel of more than 100 videos
